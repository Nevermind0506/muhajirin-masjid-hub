...
const UpcomingEvents = () => {
	const events = [
		{
			title: "Kajian Shubuh Berjama'ah",
			date: "Setiap Ahad",
			time: "04:30 (Ba'da Shubuh)",
			...
			<div className="container mx-auto px-4">
				<div className="text-center mb-12">
					...
					<p className="text-emerald-600 text-lg max-w-2xl mx-auto">
						Ikuti berbagai kegiatan keagamaan dan sosial yang rutin
					...
					{events.map((event, index) => (
						...
						>
							<CardHeader>
								<CardTitle
									className={`${
										event.highlight ? "text-white" : "text-emerald-800"
									}`}
								>
									{event.title}
									{event.highlight && (
							...
							<CardContent className="space-y-3">
								<div
									className={`flex items-center gap-2 ${
										event.highlight ? "text-emerald-100" : "text-emerald-700"
									}`}
								>
								...
								<p
									className={`${
										event.highlight ? "text-emerald-50" : "text-gray-700"
									} leading-relaxed`}
								>
									{event.description}
								</p>
